import { motion } from "framer-motion";

import { FaRegPlayCircle } from "react-icons/fa";
import { GoArrowUpRight, GoArrowDown } from "react-icons/go";

import HeroImage from "../assets/HeroImage.png";
import Inter from "../assets/interview.png";

export default function Hero() {

    const fadeUp = {
        hidden: {
            opacity: 0,
            y: 25,
        },
        visible: {
            opacity: 1,
            y: 0,
        },
    };

    return (
        <section
            id="home"
            className="relative w-full overflow-hidden md:grid md:h-screen md:grid-cols-[55%_45%]"
        >

            {/* DESKTOP BACKGROUND */}
            <motion.img
                src={HeroImage}
                alt=""
                initial={{
                    opacity: 0,
                    scale: 1.03,
                }}
                animate={{
                    opacity: 1,
                    scale: 1,
                }}
                transition={{
                    duration: 1.1,
                    ease: "easeOut",
                }}
                className="absolute inset-0 hidden h-full w-full object-cover md:block"
            />


            {/* HERO CONTENT */}
            <div className="relative z-10 flex flex-col px-5 pt-30 md:justify-center md:px-0 md:pt-0 md:pl-10">

                {/* HEADING */}
                <div className="font-five text-[2.9rem] font-bold leading-[0.95] sm:text-[4rem] md:text-[4.9em]">

                    <motion.h1
                        variants={fadeUp}
                        initial="hidden"
                        animate="visible"
                        transition={{
                            duration: 0.65,
                            ease: "easeOut",
                            delay: 0.15,
                        }}
                    >
                        Taking Web3
                    </motion.h1>


                    <div className="mt-1 flex items-baseline gap-2 md:gap-3">

                        <motion.p
                            variants={fadeUp}
                            initial="hidden"
                            animate="visible"
                            transition={{
                                duration: 0.65,
                                ease: "easeOut",
                                delay: 0.25,
                            }}
                        >
                            to the
                        </motion.p>


                        <motion.p
                            initial={{
                                opacity: 0,
                                y: 25,
                            }}
                            animate={{
                                opacity: 1,
                                y: 0,
                            }}
                            transition={{
                                duration: 0.7,
                                ease: "easeOut",
                                delay: 0.35,
                            }}
                            className="text-pink-500"
                        >
                            Streets.
                        </motion.p>

                    </div>

                </div>


                {/* SUBTEXT */}
                <motion.p
                    variants={fadeUp}
                    initial="hidden"
                    animate="visible"
                    transition={{
                        duration: 0.65,
                        ease: "easeOut",
                        delay: 0.5,
                    }}
                    className="mt-6 max-w-xl font-six text-sm font-normal leading-relaxed text-black/80 md:mt-5 md:text-base"
                >
                    Bringing Web3 products face-to-face with the people they are built for.
                    <br className="hidden md:block" />
                    {" "}
                    Creating real conversations that take Web3 beyond the screen.
                </motion.p>


                {/* CTAs */}
                <motion.div
                    variants={fadeUp}
                    initial="hidden"
                    animate="visible"
                    transition={{
                        duration: 0.65,
                        ease: "easeOut",
                        delay: 0.65,
                    }}
                    className="mt-8 flex items-center gap-7 md:gap-9"
                >

                    <motion.a
                        href="#episodes"
                        whileHover={{
                            scale: 1.04,
                        }}
                        whileTap={{
                            scale: 0.97,
                        }}
                        transition={{
                            duration: 0.2,
                        }}
                        className="flex items-center gap-2 rounded-full bg-pink-500 px-6 py-2.5 text-xs font-normal text-white"
                    >
                        <FaRegPlayCircle />

                        <p>
                            Watch Episodes
                        </p>
                    </motion.a>


                    <motion.a
                        href="#about"
                        whileHover={{
                            x: 3,
                        }}
                        transition={{
                            duration: 0.2,
                        }}
                        className="flex items-center gap-1 border-b border-black py-2.5 text-xs font-normal"
                    >
                        <p>
                            Learn More
                        </p>

                        <GoArrowUpRight />
                    </motion.a>

                </motion.div>

            </div>


            {/* MOBILE IMAGE */}
            <motion.div
                initial={{
                    opacity: 0,
                    y: 40,
                }}
                animate={{
                    opacity: 1,
                    y: 0,
                }}
                transition={{
                    duration: 0.8,
                    ease: "easeOut",
                    delay: 0.55,
                }}
                className="relative mt-12 h-[55vh] w-full md:hidden"
            >
                <img
                    src={Inter}
                    alt=""
                    className="h-full w-full object-cover"
                />
            </motion.div>


            {/* DESKTOP RIGHT */}
            <div className="relative z-10 hidden md:block"></div>


            {/* SCROLL INDICATOR */}
            <motion.a
                href="#about"
                initial={{
                    opacity: 0,
                }}
                animate={{
                    opacity: 1,
                }}
                transition={{
                    duration: 0.6,
                    delay: 1,
                }}
                className="absolute bottom-7 left-10 z-10 hidden items-center gap-2 font-six text-[0.6rem] tracking-[0.15em] text-black/40 transition-colors duration-300 hover:text-pink-500 md:flex"
            >
                <p>
                    SCROLL TO EXPLORE
                </p>

                <motion.div
                    animate={{
                        y: [0, 4, 0],
                    }}
                    transition={{
                        duration: 1.5,
                        repeat: Infinity,
                        ease: "easeInOut",
                    }}
                >
                    <GoArrowDown className="text-sm" />
                </motion.div>

            </motion.a>

        </section>
    );
}